...
class MDP(object):
    ...
    def reward_for_reach(self,target):
        ...
        reach_rew = {}
        for pair in self.MDP[1]:
            #i = np.floor_divide(pair[0],21)
            #j = pair[0] - 21*i
            #print('pair = ', pair)
            #print('i,j = ',i,j)
            #for state2 in self.MDP[1][pair][1]:
            #    i2 = np.floor_divide(state2,21)
            #    j2 = state2 - 21*i2
            #    print('i2,j2 = ',i2,j2)
    ...
    def value_evaluate(self,reward ,policy,target):
        ...
        while diff[max(diff, key=diff.get)] > eps:
            for pair in self.MDP[1]:
                ...
                for k,succ in enumerate(self.MDP[1][pair][1]):
                    ...
                    if pair[0] not in target and succ in target:
                        rew_sum += self.MDP[1][pair][0][k]
                Q_val[pair] = rew_sum + succ_sum
                #print(Q_val[pair])
            ...
            print('V_val: ' + str(diff[max(diff, key=diff.get)] ))
            #print('diff = ', diff[max(diff, key=diff.get)] )
            #print('diff - eps = ', diff[max(diff, key=diff.get)] - eps )
            #print('Val of 0 = ', V_val[0])
            #print('diff = ', diff[max(diff, key=diff.get)])
        return V_val, Q_val
    ...
    def compute_max_reach_value_and_policy(self,init, target, absorbing):
     ...
     # target: the 'list' of ABSORBING target states
        ...
        m.setParam( 'DualReductions', False)
        ...
        #print('total prob  = ', total_reach_prob)
        ...
        for state in range(len(self.states())):
            ...
            for action in self.active_actions()[state]:
                ...
                if X[state,act_ind].x >= 1e-4:
                    policy[state] = action
        V_val,_=self.value_evaluate(reach_rew,policy,target)
        max_reach_val=V_val[init]
        #max_reach_val = total_reach_prob
        print(max_reach_val)
